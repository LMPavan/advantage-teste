    ...
    <div className="inline-form">
      ...
      <div className="field">
        ...
        <select className="input" value={attendantId} onChange={(e) => setAttendantId(e.target.value)}>
          ...
          {showOwnOption && ownUserId && <option value={ownUserId}>Eu mesmo (gerente) — meta pessoal</option>}
        </select>
      </div>
      <div className="field">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {station?.managerCanManageGoals !== false ? (
            <NewGoalForm
              ...
              ownUserId={user?.id}
              showOwnOption={station?.managerCommissionMode === "CUSTOM"}
              onCreated={load}
            />
          ) : (
          ...
          <tbody>
            {goals.map((g) => (
              <tr key={g.id}>
                ...
                <td>{g.attendantId === user?.id && <ManagerEntryForm goal={g} onSaved={load} />}</td>
              </tr>
            ))}
          </tbody>
        </table>
